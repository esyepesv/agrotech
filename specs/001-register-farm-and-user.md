...
- **Campos de USUARIO:** tipo (administrador-dueño / trabajador); tipo de identificación (CC / CE / pasaporte); número de identificación; # celular WhatsApp (o Telegram); correo electrónico (opcional).
...
2. El bot pregunta primero el **rol**, con botones: `[Soy dueño]` `[Soy trabajador]`.
3. **Rama dueño** — pide, un campo por mensaje, en este orden: nombre de la finca → tipo de persona `[Natural]` `[Jurídica]` → cédula o NIT (según tipo) → ubicación → capacidad de ceba → capacidad de cría → capacidad total → registro sanitario ICA → tipo de identificación del usuario `[Cédula]` `[Extranjería]` `[Pasaporte]` → número de identificación → correo `[Escribirlo]` `[No tengo]` `[Después]`. El **celular no se pregunta**: es el `channelUserId` del canal.
...
- **Mapeo por canal:** WhatsApp → *reply buttons* (máx. **3** por mensaje, etiqueta ≤ 20 caracteres) y *list message* para la búsqueda de fincas (hasta 10 filas). Telegram → *inline keyboard* (una fila por opción; `callback_data` ≤ 64 bytes). Todos los pasos de este spec caben en 3 opciones, así que ningún paso necesita paginación.
...
- **Paso 1 — Cuenta:** tipo de identificación (CC/CE/PA), número, celular (validación: 10 dígitos colombianos empezando por 3), correo opcional.
...
- El dueño ve y resuelve solicitudes pendientes por chat: al escribir al bot, si tiene solicitudes, el sistema se las presenta ("Fulano (CC 1032456789) pide unirse a Finca La Esperanza. ¿Apruebas?") → `ApproveWorker`.
